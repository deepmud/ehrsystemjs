...
export default   function  DashboardPage() {
  ...
  const id = params.id;
    ...
    const { data  , error, isLoading } =  useSWR<GetListDashboardAppointmentDtoDto>(isAuthenticated() ? `/MedicalRecord/FetchDepartmentAppointments` : null, getListDashboardAppointmentDtoDto);
                        ...
                        <table className="w-full">
                                ...
                                data.listDashboardappointmentlistDto.map((p: ListDashboardAppointmentDto) => (
                                    <tr className="border-b dark:border-gray-700" key={p.id}>
                                        ...
                                        { user?.department == "RE" || user?.department == "NU"   ? " " : <td className="py-3"><button className="px-3 py-1 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-lg text-sm"  onClick={() => router.push( p.department == "Doctor" ? `/doctor/getdiagnosis/${p.id}` : `/pharmacy/getdrugs/${p.id}`)}>view</button></td>}
                                    </tr>
                                    )) :
